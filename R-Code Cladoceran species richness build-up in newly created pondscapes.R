...
Physicochemistrydata <- read.csv("Physicochemistrydata.csv")
cumsumspec <- read.csv("Cumsumspec.csv")
inSR <- read.csv("insSR.csv")
presenceabsence <- read.csv("Presenceabsence.csv")
...
Physicochemistrydata <- Physicochemistrydata %>%
  mutate(Macrophyte = if_else(Emerg != 0 | Subm != 0 | Float != 0, "Present", "Absent"))

# PCA Preparation #
fysico_pca_data <- Physicochemistrydata %>%
...
ponds_met_emerg_df <- Physicochemistrydata %>%
  filter(Emerg >= 0.25) %>%
  distinct(POND) %>%
  arrange(POND)
...
long_data <- long_data %>%
  ...
  left_join(datahydromacro %>% select(POND, Hydroperiod), by = c("pond" = "POND"))
...
long_data <- long_data %>%
  left_join(Physicochemistrydata %>% select(Visit, POND, Emerg),
            by = c("Visit", "pond" = "POND"))
...
presenceabsence_matrix <- presenceabsence |>
  mutate(across(-X, ~ ifelse(.x > 0, 1, 0))) |>
  select(-X) |>
  as.matrix()

nodf_result <- nestednodf(presenceabsence_matrix)
nodf_result

set.seed(123) # for reproducibility

# Simulate 1000 randomized matrices and calculate NODF for each
null_models <- replicate(1000, {
  randomized <- permatfull(presenceabsence_matrix, mtype = "prab", fixedmar = "rows", shuffle = "both", times = 1)
  randomized_matrix <- randomized$perm[[1]] # Extract the first permuted matrix
  nestednodf(randomized_matrix, order = TRUE, weighted = FALSE)$statistic["NODF"]
})

# Calculate p-value
p_value <- mean(null_models >= nodf_result$statistic["NODF"])

# Plot the null distribution
hist(null_models, main = "Null distribution of NODF", xlab = "NODF")
abline(v = nodf_result$statistic["NODF"], col = "red", lwd = 2)
